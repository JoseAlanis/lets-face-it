...
# Target    : Voxel Count on Conjunction Faces vs Houses, Faces vs Scrambled
...
# import csv
fname <- './LI_results_indiv-maxima_ROI-10mm_tthreshold_001unc_voxel-count.csv'
data <- read.table(fname,
                   header = T, sep = ',', 
                   na.strings = c(9999.0000))
# drop `data$notes`
data <- data %>% 
  select(!notes)
...
li_plot <- ggplot(data,
  ...
  guides(fill = "none",
         ...
         shape = "none"); li_plot
ggsave(filename = './LI_plot_10mm_hand_voxel_count.tiff',
       plot = li_plot, 
...
int_plot <- ggplot(data = data, 
  ...
  labs(y = 'Observed LI-Value', x = '',
       ...
       shape = 'Sex'); int_plot
ggsave(filename = './interaction_plot_area_sex_hand_voxel_count.tiff',
       plot = int_plot, 
...
data %>% 
  ...
  tab_df(file = './overall_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T), 
            ...
            med = median(value, na.rm=T)) %>%
  as.data.frame() %>%
  tab_df(file = './sex_descriptives_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T), 
            sd = sd(value, na.rm=T),
            med = median(value, na.rm=T)) %>%
  tab_df(file = './hand_descriptives_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T), 
            sd = sd(value, na.rm=T),
            med = median(value, na.rm=T)) %>%
  tab_df(file = './area_hand_descriptives_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T), 
            sd = sd(value, na.rm=T),
            med = median(value, na.rm=T)) %>%
  tab_df(file = './area_descriptives_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T),
            sd = sd(value, na.rm=T),
            med = median(value, na.rm=T)) %>%
  tab_df(file = './hand_sex_descriptives_voxel_count.html', digits = 3)
...
data %>% 
  ...
  summarise(m = mean(value, na.rm=T),
            sd = sd(value, na.rm=T),
            med = median(value, na.rm=T)) %>%
  tab_df(file = './hand_sex_area_descriptives_voxel_count.html', digits = 3)
...
anova_mod_gamma %>%
  apa("ANOVA table overall LI-model: Gamma model for LI values (voxel count)") %>%
  fmt_number(columns = c(2, 4), decimals = 4) %>%
  gtsave(
    "./anova_mod_gamma_overall_voxel_count.html", inline_css = TRUE)
tiff(filename = './gamma_overall_model_diagnostics_voxel_count.tiff',
     width = 15, height = 20, units = 'cm', res =  1200)
check_model(mod_gamma_li)
dev.off()
model_performance(mod_gamma_li) %>%
  apa("Overall Gamma LI-model performance (voxel count)", stub = F) %>%
  ...
  gtsave(
    "./mod_gamma_overall_performance_voxel_count.html", inline_css = TRUE)
standardize_parameters(mod_gamma_li) %>%
  apa("Standardized beta coefficients: 
      Overall Gamma LI-model (voxel count)", stub = F) %>%
  ...
  tab_footnote(
    ...
    locations = cells_body(
      ...
      rows = 6)) %>%
  gtsave(
    "./standardized_beta_mod_gamma_voxel_count.html", inline_css = TRUE)
...
anova_mod_gamma_rh %>%
  apa("ANOVA table right handers' LI-model: 
      Gamma model for LI values from right handed participants (voxel count)") %>%
  fmt_number(columns = c(2, 4), decimals = 4) %>%
  gtsave(
    "./anova_mod_gamma_right_handers_voxel_count.html", inline_css = TRUE)
...
anova_mod_gamma_lh %>%
  apa("ANOVA table left handers' LI-model: 
      Gamma model for LI values from left handed participants (voxel count)") %>%
  fmt_number(columns = c(2, 4), decimals = 4) %>%
  gtsave(
    "./anova_mod_gamma_left_handers_voxel_count.html", inline_css = TRUE)
...
anova_mod_ffa %>%
  apa("ANOVA table FFA LI-model (voxel count)") %>%
  fmt_number(columns = c(2, 5), decimals = 4) %>%
  tab_source_note(
    source_note = 
  ...
  gtsave(
    "./anova_mod_ffa_voxel_count.html", inline_css = TRUE)

standardize_parameters(mod_ffa) %>%
  apa("Standardized beta coefficients: 
      FFA LI-model (voxel count)", stub = F) %>%
  fmt_number(columns = c(2:5), decimals = 4) %>%
  tab_footnote(
    footnote = "age, centred around zero",
    locations = cells_body(
  ...
  tab_footnote(
    ...
    locations = cells_body(
      ...
      rows = 4))  %>%
  gtsave(
    "./standardized_beta_mod_ffa_voxel_count.html", inline_css = TRUE)
...
ffa_means %>%
  data.frame() %>%
  apa("Estimated marginal means (emmeans) for FFA model (voxel count)",
      stub = F) %>%
  fmt_number(columns = c(3:7), decimals = 4) %>%
  gtsave(
    "./emmeans_mod_ffa_voxel_count.html", inline_css = TRUE)

contrast(ffa_means, 'tukey', adjust = 'fdr') %>%
  data.frame() %>%
  apa("Pairwise contrasts for FFA model (voxel count)",
      stub = F) %>%
  fmt_number(columns = c(3:7), decimals = 4) %>%
  gtsave(
    "./contrasts_mod_ffa_voxel_count.html", inline_css = TRUE)
